...
MAXT = 18;  @ 1974 to 1990 @
MAXN = 10;  @ 10 Asian countries @
...
@ * JASA: Application 2: Energy demand in Asia @

@ * Load the data matrix. @

    load eda[temp,4] = eda.dat;

    AERP  = eda[.,1];  @ average energy real price @
    POPUL = eda[.,2];  @ population @
    TOTC  = eda[.,3];  @ total energy consumption @
    RGDPL = eda[.,4];  @ real per capita GDP @

@*  Load group names (alphanumeric) or codes (numeric) if any.
**  The file dimension MUST be MAXN*1. If there are no group names/codes,
**  leave this space blank. You MUST call this "GRNAME". @

   load GRNAME[maxn,1] = j2name.dat; @ Country codes: alpahnumeric @
...
@* Selecting the stochastic (X) regressors @
...
format /rd 3,0;
"PAGE "  page;"";
PRSUM(cflag,iter,sub_op,n_pos,dem_op,ml_op,ini_op,RSLL,URSLL,n_LR,n,T,ms_op,
      Alag,COMPRES,maxlag,nu_op,PMGtheta,sm_op);
...
IF     ml_op eq 1;
       ...
       k,nn,T,numsi,sub_op,n_subgr,grname,na_subgr,gn_op1,gn_op2,yname,page);

ELSEIF ml_op eq 2;
       {page,PMLESUM} = PRGROUP2(OLSRES,PMLERES,CFNAME,OLSDRES,PMLEDRES,maxlag,
     k1,k2,nn,T,numsi,sub_op,n_subgr,grname,na_subgr,gn_op1,gn_op2,yname,page);
